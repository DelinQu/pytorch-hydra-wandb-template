...
def main(cfg):
    ...
    if cfg.n_gpu > n_gpu:
        cfg.n_gpu = n_gpu
        logger.info('available gpu number is {}, set n_gpu to {}'.format(n_gpu, cfg['n_gpu']))

    config = OmegaConf.to_yaml(cfg, resolve=True)
    torch.multiprocessing.spawn(init_worker, nprocs=cfg.n_gpu, args=(cfg.n_gpu, working_dir, config))
